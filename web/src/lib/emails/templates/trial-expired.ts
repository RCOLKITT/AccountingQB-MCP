...
export function trialExpiredEmail(params: TrialExpiredEmailParams): {
  ...
  const content = `
    ...
    ${paragraph("Want the full 132 tools back — writes, tax prep, and the deduction finder? Pick a plan anytime:")}
  ...
  return {
    ...
    html: emailWrapper(
      content,
      `Your trial has ended — you're on the free plan; pick a plan anytime to unlock all 132 tools`,
    ),
  };
}
