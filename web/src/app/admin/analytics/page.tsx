    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-400 mt-1 text-sm">
            External visitors only (your team &amp; QA traffic filtered out).
            Tag campaign links with{" "}
            <code className="text-gray-500">?utm_source=…</code>.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:max-w-lg">
        <BigMetric
          ...
          value={a.current.visitors}
          change={vCh}
          accent
        />
        <BigMetric
          label={`Pageviews · ${days}d`}
          value={a.current.views}
          change={pCh}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#131a2e] rounded-xl border border-white/10 p-6">
          <h2 className="text-sm font-semibold text-white">
            Pre-signup funnel · {days}d
          </h2>
          <p className="text-xs text-gray-500 mt-1">
            Site conversion:{" "}
            <span className="text-cyan-400 font-semibold">{siteConv}%</span> of
            visitors sign up.
          </p>
          ...
          <p className="text-xs text-gray-600 mt-4">
            ...
            >
              activation → paid continues in the funnel →
            </Link>
          </p>
        </div>
        ...
        <div className="bg-[#131a2e] rounded-xl border border-white/10 p-6">
          <h2 className="text-sm font-semibold text-white">
            New vs returning · {days}d
          </h2>
          <p className="text-xs text-gray-500 mt-1">
            Returning visitors are a sign your content &amp; retargeting are
            landing.
          </p>
          <div className="mt-4 flex h-6 w-full overflow-hidden rounded-lg">
            <div
              className="bg-cyan-500/60 flex items-center justify-center text-[10px] text-white"
              style={{ width: `${(nr.newVisitors / nrTotal) * 100}%` }}
            >
              {nr.newVisitors > 0 ? "new" : ""}
            </div>
            <div
              className="bg-blue-600/50 flex items-center justify-center text-[10px] text-white"
              style={{ width: `${(nr.returning / nrTotal) * 100}%` }}
            >
      ...
      <div className="bg-[#131a2e] rounded-xl border border-white/10 p-6">
        ...
        {a.trend.length === 0 ? (
          <p className="text-sm text-gray-500">
            No external pageviews yet in this window.
          </p>
        ) : (
              ...
              >
                ...
                <div className="w-full flex-1 flex items-end">
                  <div
                    ...
                    style={{
                      height: `${Math.max(2, (t.views / maxTrend) * 100)}%`,
                    }}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <BarPanel
          ...
          rows={a.pages.map((p) => ({ label: p.path, value: p.views }))}
          mono
        />
        <BarPanel
          title={`Traffic sources · ${days}d`}
          rows={a.sources.map((s) => ({ label: s.source, value: s.views }))}
        />
        <BarPanel title={`Geography · ${days}d`} rows={a.geo} unit="visitors" />
        <BarPanel
          title={`Devices · ${days}d`}
          rows={a.devices}
          unit="visitors"
        />
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline gap-2 mt-1">
        <p
          className={`text-3xl font-bold ${accent ? "text-cyan-300" : "text-white"}`}
        >
          {value.toLocaleString("en-US")}
        </p>
        <span className={`text-sm ${color}`}>
          {change.up === true && change.txt !== "new"
            ? "↑ "
            : change.up === false
              ? "↓ "
              : ""}
        ...
        {rows.length === 0 ? (
          ...
          rows.map((r) => (
            <div key={r.label} className="flex items-center gap-3 py-1.5">
              <span
                className={`text-sm text-gray-300 flex-1 truncate ${mono ? "font-mono" : ""}`}
              >
                {r.label}
              </span>
