...
export default function ComposePage() {
  ...
  const aiDraft = async () => {
    if (!goal.trim()) {
      setStatus("Describe the campaign goal first.");
      return;
    }
    setDrafting(true);
    setStatus(null);
  ...
  const call = async (mode: "test" | "dryRun" | "send") => {
    ...
    try {
      ...
      if (mode === "test")
        setStatus(`✓ Test sent to ${data.to}. Check your inbox.`);
      else if (mode === "dryRun")
        setStatus(
          `${data.eligible} eligible recipients in "${cohort}" (${data.suppressed} unsubscribed, excluded).`,
        );
      else
        setStatus(
          `✓ Scheduled ${data.scheduled} emails to "${cohort}" — sending ~${perHour}/hr over ~${data.approxHours}h.`,
        );
    } catch {
      setStatus("Network error.");
    } finally {
  ...
  const schedule = async () => {
    ...
    if (!dry.eligible) {
      setStatus("No eligible recipients.");
      return;
    }
    if (
      !window.confirm(
        `Send this to ${dry.eligible} people in "${cohort}"? They'll go out ~${perHour}/hr. This is a real send.`,
      )
    )
      return;
    call("send");
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 mt-1">
          ...
          &amp; unsubscribers are auto-excluded; every send carries a compliant
          footer.
        </p>
      </div>
          ...
          <div className="bg-[#0a0e1a] rounded-xl border border-white/10 overflow-hidden">
            ...
            <div className="p-6 text-center">
              ...
              <div className="bg-[#131a2e] rounded-xl border border-white/10 p-6 text-left">
                ...
                {draft.ctaText && (
                  <div className="my-4">
                    <span className="inline-block rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 px-5 py-2 text-white text-sm font-medium">
                      {draft.ctaText}
                    </span>
                  </div>
                )}
                {draft.signoff && (
                  <p className="text-gray-400 text-sm mt-4">{draft.signoff}</p>
                )}
              </div>
              <p className="text-[10px] text-gray-600 mt-3">
                Unsubscribe + mailing address are appended automatically on
                send.
              </p>
            </div>
          </div>
...
const inp =
  "w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-gray-500 focus:border-cyan-500/50 focus:outline-none";
