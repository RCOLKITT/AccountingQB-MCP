...
def test_prior_only_line_merged_not_appended():
    cur = ["Sales", "Total Income", "Advertising", "Rent",
           "Total Expenses", "Net Income"]
    prior = ["Sales", "Total Income", "Advertising", "Consulting", "Rent",
             "Total Expenses", "Net Income"]
    ...
    assert (merged.index("Advertising") < merged.index("Consulting")
...
def test_report_tools_have_optional_dates():
    ...
    for name in ("qb_balance_sheet", "qb_cash_flow", "qb_trial_balance",
                 "qb_ar_aging", "qb_ap_aging", "qb_expense_summary",
                 "qb_income_summary", "qb_sales_tax_summary", "qb_tax_summary",
                 "qb_list_deposits", "qb_vendor_summary"):
        fn = getattr(s, name)
