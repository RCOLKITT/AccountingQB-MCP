...
def test_classify_matches_review_numbers(monkeypatch):
    ...
    out = asyncio.run(
        s.qb_stripe_reconcile("2026-07", REPORT, expected_ending_balance=112.04)
    )
...
def test_tie_out_off_blocks(monkeypatch):
    _no_accounts(monkeypatch)
    out = asyncio.run(
        s.qb_stripe_reconcile(
            "2026-07", REPORT, dry_run=False, expected_ending_balance=100.00
        )
    )
...
def test_posts_balanced_entry(monkeypatch):
    ...
    out = asyncio.run(
        s.qb_stripe_reconcile(
            "2026-07", REPORT, dry_run=False, expected_ending_balance=112.04
        )
    )
...
def test_detects_manual_entry_in_period(monkeypatch):
    ...
    accts = {
        "Stripe Clearing": {"Id": "155", "Name": "Stripe Clearing"},
        "Sales": {"Id": "5", "Name": "Sales"},
        "Merchant Fees": {"Id": "60", "Name": "Merchant Fees"},
        "Software & Apps": {"Id": "8", "Name": "Software & Apps"},
        "Refunds": {"Id": "70", "Name": "Refunds"},
            ...
            return {
                "QueryResponse": {
                    "JournalEntry": [
                        {
                            "Id": "888",
                            "PrivateNote": "manual clearing cleanup",
                            "Line": [
                                {
                                    "JournalEntryLineDetail": {
                                        "AccountRef": {"value": "155"}
