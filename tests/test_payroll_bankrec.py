        ...
        if "ProfitAndLoss" in endpoint:
            return {"Rows": {"Row": [
                {"Header": {"ColData": [{"value": "Expenses"}]},
                 "Rows": {"Row": [
                     {"ColData": [{"value": "Wages & Salaries"}, {"value": "820.00"}]},
                     {"ColData": [{"value": "Rent"}, {"value": "1200.00"}]},
                 ]},
                 "Summary": {"ColData": [{"value": "Total Expenses"}, {"value": "2020.00"}]}}]}}
        return {}
    ...
    async def fake_query(q, **kw):
        ...
        if "Liability" in q:
            return {"QueryResponse": {"Account": [
                {"Name": "Payroll Tax Payable", "CurrentBalance": 63.0}]}}
    ...
    async def fake_query(q, **kw):
        if "FROM Purchase" in q:
            return {"QueryResponse": {"Purchase": [
                {"Id": "1", "TxnDate": "2026-03-01", "TotalAmt": 12.50,
                 "EntityRef": {"name": "Coffee Shop"}},
                {"Id": "2", "TxnDate": "2026-03-02", "TotalAmt": 340.00,
                 "EntityRef": {"name": "AWS"}},
                {"Id": "3", "TxnDate": "2026-03-05", "TotalAmt": 500.00,
                 "EntityRef": {"name": "Uncleared Vendor"}},  # in books, not on stmt
            ]}}
        return {"QueryResponse": {}}
