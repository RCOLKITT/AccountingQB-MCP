...
def test_account_transactions_counts_all_230(monkeypatch):
    ...
    purchases = [{"Id": str(i), "TxnDate": "2026-06-01", "TotalAmt": 10.0,
                  "AccountRef": {"value": "77"}} for i in range(1, 231)]

    async def fake_query(q):  # account lookup (unpaginated) + others empty
        if "FROM Account" in q:
            return {"QueryResponse": {"Account": [
                {"Id": "77", "Name": "Delta Platinum Business Card",
                 "AccountType": "Credit Card", "Active": True, "CurrentBalance": 0}]}}
    ...
    out = asyncio.run(s.qb_account_transactions(
...
def test_list_accounts_paginates_and_filters(monkeypatch):
    ...
    accts = ([{"Id": str(i), "Name": f"Acct{i}", "AccountType": "Expense",
               "Active": True} for i in range(1, 109)]
             + [{"Id": "200", "Name": "Checking", "AccountType": "Bank", "Active": True},
                {"Id": "201", "Name": "Delta Card", "AccountType": "Credit Card", "Active": True}])
...
def test_search_finds_bank_feed_line_description(monkeypatch):
    ...
    purchases = [{"Id": "1", "TxnDate": "2026-05-01", "TotalAmt": 600.0,
                  "PrivateNote": "", "EntityRef": {"name": "Amex"},
    ...
    out = asyncio.run(s.qb_search_transactions(
        "2026-01-01", "2026-12-31", "MOBILE PAYMENT"))
