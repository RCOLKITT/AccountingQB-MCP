...
def _report(cols, groups):
    ...
    for label, datarows, summ in groups:
        sec = {"Header": {"ColData": [{"value": label}]},
               "Rows": {"Row": [{"ColData": [{"value": v} for v in r]}
                                for r in datarows]}}
        if summ:
...
def test_format_report_table_keeps_all_columns():
    rep = _report(["Date", "Transaction Type", "Amount"], [
        ("Acme Co", [["2026-01-05", "Invoice", "1,000.00"],
...
def test_report_tools_use_exact_endpoint_names(monkeypatch):
    ...
    cases = [
        (lambda: s.qb_sales_by_customer("2026-01-01", "2026-12-31"), "reports/SalesByCustomer"),
        (lambda: s.qb_sales_by_product("2026-01-01", "2026-12-31"), "reports/SalesByProduct"),
        (lambda: s.qb_profit_loss_detail("2026-01-01", "2026-12-31"), "reports/ProfitAndLossDetail"),
        (lambda: s.qb_transaction_list("2026-01-01", "2026-12-31"), "reports/TransactionList"),
        (lambda: s.qb_customer_balance_detail("2026-06-30"), "reports/CustomerBalanceDetail"),
        (lambda: s.qb_vendor_balance_detail("2026-06-30"), "reports/VendorBalanceDetail"),
    ]
