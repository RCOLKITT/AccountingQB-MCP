...
|---|---|---|
| 00 | Cover | `qb_company_info` + `qb_books_health_audit` + organizer answers + payroll disclosure |
| 01 | Trial Balance | `qb_trial_balance(start, end)` |
| 02 | Reconciliation | `qb_reconciliation_status(end)` — keep its API-limitation caveat verbatim |
| 03 | Profit & Loss (Comparative) | `qb_comparative_statements("pl", year)` |
| 04 | Balance Sheet (Comparative) | `qb_comparative_statements("bs", year)` |
| 05 | Cash Flow | `qb_cash_flow(start, end)` |
| 06 | General Ledger | `qb_general_ledger(start, end)` |
| 07 | Tax Mapping | `qb_schedule_c_detailed(year)` / `qb_t2125_summary(year)` |
| 08 | Contractors | `qb_1099_contractor_report(year)` / `qb_t4a_contractor_report(year)` |
| 09 | Sales Tax | `qb_sales_tax_summary(start, end)` / `qb_gst_hst_return(start, end)` |
| 10 | Fixed Assets | `qb_depreciation_schedule(year)` / `qb_cca_schedule(year=year)` |
| 11 | Tax Payments Made | `qb_tax_payments_made(year)` |
| 12 | Owner's Draws | `qb_owner_draws(year)` |
| 13 | Open Items | `qb_uncategorized_transactions` + `qb_find_duplicates` + `qb_unknown_vendor_report` (all with start/end) |
...
2. **Cover sheet (00)** contains: company name, period, prepared date,
   books-health score, open-items count, the user's tax-organizer answers if provided (miles, home office, premiums, loans, assets, estimated-payment confirmation, unusual items), the payroll disclosure ('Payroll: not included — attach W-2/941 (US) or T4 summaries (CA) from your payroll provider'),
...
4. **File naming**: a single Excel workbook
   ...
   produce a folder of CSVs named `00-Cover.csv` … `13-Open-Items.csv`
