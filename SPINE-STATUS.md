...
| Tests (web) | 🟢 | `npm run test:unit` (vitest) + `npm run test:e2e` (Playwright) | **Unit:** `src/lib/*.test.ts` — token-crypto fail-closed + round-trip (BLOCKING in `web-checks`). **E2E:** | Public-route smoke + **auth-boundary** (protected API/admin routes reject anon; no anon writes) + **authed happy-path** (a real signed-in Clerk user renders the dashboard) on a real prod build; in CI (`web-e2e`, required). Authed spec uses a Clerk **dev-instance** `+clerk_test` user via `@clerk/testing` (creds: Doppler → GitHub secrets); it skips cleanly when creds are absent (fork PRs), never touches the `pk_live_` prod instance. |
